...
- **PR templates (DESIGN-0013, IMPL-0012 Phase 4)** — operators may declare `pr { title body labels inherits }` blocks at three scopes: `defaults { pr {} }`, `rule "file" "..." { pr {} }`, and `reconcile "..." { pr {} }`. Title/Body strings are compiled into `*template.Compiled` at policy-load time by `internal/policy/pr.go.compilePolicyTemplates`; parse errors fail load with a location-prefixed message. Resolution: `ResolveRulePR(rule, defaults)` and `ResolveReconcilerPR(reconciler, defaults)` perform field-by-field merge; child wins on every explicitly-set field, unset fields inherit from parent only when `inherits=true`. `inherits=false` short-circuits parent inheritance. `labels = []` is an explicit empty-list override (tracked via sidecar `LabelsSet bool` because HCL `[]string` decode loses the absent-vs-empty-list distinction). Reconciler PRs deliberately skip `rule.pr` — they merge `reconciler.pr → defaults.pr` only.
